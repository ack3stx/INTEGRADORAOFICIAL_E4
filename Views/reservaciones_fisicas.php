...
SELECT tipos.tipo, t_habitacion.nombre, COUNT(habitacion.id_habitacion) AS cantidad_habitaciones
FROM tipos
LEFT JOIN habitacion ON tipos.tipo = habitacion.tipo_habitacion AND habitacion.id_habitacion NOT IN (SELECT habitacion.id_habitacion
FROM habitacion
INNER JOIN detalle_reservacion ON habitacion.id_habitacion = detalle_reservacion.habitacion
WHERE
(detalle_reservacion.FECHA_INICIO <= '$fechaFinConvertida' AND detalle_reservacion.FECHA_FIN >= '$fechaInicioConvertida'))
LEFT JOIN t_habitacion ON t_habitacion.id_tipo_habitacion = tipos.tipo
GROUP BY tipos.tipo, t_habitacion.nombre
ORDER BY tipos.tipo";

        $array=$db->seleccionar($consultilla);

        $cant_dobles=$array[0]->cantidad_habitaciones;
        $cant_king=$array[1]->cantidad_habitaciones;
        $cant_sencilla=$array[2]->cantidad_habitaciones;
