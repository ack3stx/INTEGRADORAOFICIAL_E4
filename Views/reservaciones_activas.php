    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            for ($i = 0; $i < count($detalleIds); $i++) {
                ...
                $consultaActual = "
                    SELECT habitacion, fecha_inicio, fecha_fin, habitacion.NUM_HABITACION 
                ...
                $consultaDisponibilidad = "
                    CALL Verificar_Disponibilidad_Habitacion($numHabitacion, '$nuevaFechaFin')";
                ...
                if ($resultadoDisponibilidad[0]->Disponibilidad === 'No Disponible') {
                    throw new Exception("No hay disponibilidad para la habitación en las fechas seleccionadas.");
                }

                // Obtener precio de la habitación
                $idHabitacion = $detalleActual[0]->habitacion;
                $consultaPrecio = "
                    SELECT t_habitacion.PRECIO
                    FROM habitacion
                    JOIN t_habitacion ON t_habitacion.ID_TIPO_HABITACION = habitacion.TIPO_HABITACION
                    WHERE habitacion.ID_HABITACION = $idHabitacion";
                $precioHabitacion = $db->seleccionar($consultaPrecio);

                // Calcular la diferencia de días entre las fechas
                $fechaInicio = new DateTime($detalleActual[0]->fecha_fin);
                $nuevaFechaFin = new DateTime($nuevaFechaFin);
                $diasEstancia = $fechaInicio->diff($nuevaFechaFin)->days;

                // Calcular el costo total
                $costoTotal += $precioHabitacion[0]->PRECIO * $diasEstancia;
                $diasEstanciaTotal += $diasEstancia;
            }

            $db->desconectarBD();
            header("Location: pagina_de_pago.php?costoTotal=$costoTotal&diasEstancia=$diasEstanciaTotal&precioHabitacion=".$precioHabitacion[0]->PRECIO);
            exit;
        } catch (Exception $e) {
            $db->desconectarBD();
        ...
        // Consulta para obtener reservaciones activas
        $consulta = "
            SELECT 
